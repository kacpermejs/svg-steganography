...
def modify_floats_in_svg_2(input_file_path, output_file_path, replacement_function, message):
    ...
    partitioned_list = [message_list[i:i + 1] for i in range(0, len(message_list))]
    ...
    reminder = deque(partitioned_list)
    ...
    for match in float_pattern.finditer(svg_content):
        ...
        modified_content.append(svg_content[last_end:match.start()])
        # Replace the current match
        replacement, reminder = replacement_function(reminder, match)
        modified_content.append(replacement)
        # Update the last_end to the end of the current match
        last_end = match.end()
